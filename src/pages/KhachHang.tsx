...
import { Users, Search, DollarSign, Plus, X, Phone, Calendar, Check, Store } from 'lucide-react';
...
export default function KhachHang() {
  ...
  const [selectedStore, setSelectedStore] = useState<string>(
    state.currentUser?.role === 'SALE' && state.currentUser?.storeId ? state.currentUser.storeId : 'ALL'
  );
  ...
  const [newCustomer, setNewCustomer] = useState<Partial<Customer>>({
    ...
    address: '',
    notes: '',
    group: state.customerGroups.length > 0 ? state.customerGroups[0].id : 'VANG_LAI'
  });
  const [newCustomerGroup, setNewCustomerGroup] = useState({ name: '', color: '#00f3ff' });
  ...
  const totalDebtSystem = useMemo(() => {
    ...
    return state.customers.reduce((sum, c) => sum + (c.storeDebts?.[selectedStore] || 0), 0);
  }, [state.customers, selectedStore]);

  const handleOpenDebtModal = (customer: Customer) => {
    setSelectedCustomer(customer);
    const maxDebt = selectedStore === 'ALL' ? customer.totalDebt : (customer.storeDebts?.[selectedStore] || 0);
  ...
  const handleCollectDebt = (e: React.FormEvent) => {
    ...
    const currentMaxDebt = debtPayment.storeId === 'KHO_TONG' || debtPayment.storeId === 'ALL' ? selectedCustomer.totalDebt : (selectedCustomer.storeDebts?.[debtPayment.storeId] || 0);
    
    // Allow collection if it doesn't exceed total debt
    if (amount <= 0 || amount > selectedCustomer.totalDebt || amount > currentMaxDebt) {
    ...
    dispatch({
      ...
      payload: {
        ...
        storeDebts: {
          ...
          [debtPayment.storeId]: Math.max(0, (selectedCustomer.storeDebts?.[debtPayment.storeId] || 0) - amount)
        }
      }
    });

    toast.success(`Thu nợ thành công cho ${SHOP_LABELS[debtPayment.storeId] || debtPayment.storeId}!`);
  ...
  const handleCreateCustomer = (e: React.FormEvent) => {
    ...
    setNewCustomer({ name: '', phone: '', address: '', notes: '', group: state.customerGroups.length > 0 ? state.customerGroups[0].id : 'VANG_LAI' });
  };
  ...
  const handleCreateCustomerGroup = (e: React.FormEvent) => {
    ...
    dispatch({
      ...
      payload: { id: groupId, name: newCustomerGroup.name, color: newCustomerGroup.color }
    });
    ...
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div>
          ...
          <p className="text-dark-muted text-sm mt-1">Quản lý thông tin và công nợ khách hàng</p>
        </div>
        ...
        <div className="flex flex-col sm:flex-row items-center gap-4 w-full sm:w-auto">
          ...
          <div className="w-full sm:w-auto bg-dark-card border border-red-500/30 rounded-lg px-4 py-2 flex items-center shadow-[0_0_10px_rgba(239,68,68,0.1)]">
            ...
            <div>
              <p className="text-xs text-dark-muted">{selectedStore === 'ALL' ? 'Tổng nợ hệ thống' : `Nợ tại ${SHOP_LABELS[selectedStore]}`}</p>
              <p className="text-lg font-bold text-red-400">{totalDebtSystem.toLocaleString()}đ</p>
            </div>
          </div>
        </div>
      </div>
            ...
            <tbody className="divide-y divide-dark-border">
              {customers.length === 0 ? (
                ...
                customers.map(customer => {
                  const spent = selectedStore === 'ALL' ? customer.totalSpent : (customer.storeSpent?.[selectedStore] || 0);
                  const debt = selectedStore === 'ALL' ? customer.totalDebt : (customer.storeDebts?.[selectedStore] || 0);
                  
                  return (
                  <tr key={customer.id} className="hover:bg-dark-bg/50 transition-colors">
                    <td className="px-6 py-4 font-medium">{customer.name}</td>
                    <td className="px-6 py-4">
                      {(() => {
                        const grp = state.customerGroups.find(g => g.id === customer.group);
                        if (grp) {
                          return (
                            <span 
                        ...
                        return <span className="px-2 py-1 text-[10px] rounded bg-gray-500/20 text-gray-400 font-bold border border-gray-500/30">VÃNG LAI</span>;
                      })()}
                    </td>
                    <td className="px-6 py-4">
                    ...
                    <td className="px-6 py-4 text-right text-dark-muted font-medium">
                      ...
                      {selectedStore === 'ALL' && <div className="text-[10px] text-neon-cyan/70 mt-1">{customer.points || 0} điểm</div>}
                    </td>
                    <td className="px-6 py-4 text-right">
                      {debt && debt > 0 ? (
                        <span className="font-bold text-red-400">{debt.toLocaleString()}đ</span>
                      ) : (
                    ...
                    <td className="px-6 py-4 text-center">
                      {debt && debt > 0 ? (
                        ...
                        <span className="text-xs text-dark-muted px-3 py-1">Hết nợ</span>
                      )}
                    </td>
                  </tr>
                )})
          ...
          <div className="bg-dark-card border border-dark-border rounded-xl w-full max-w-md shadow-2xl overflow-hidden">
            ...
            <form onSubmit={handleCollectDebt} className="p-4 space-y-4">
              <div className="bg-dark-bg p-3 rounded-lg border border-dark-border mb-4">
                <p className="text-sm text-dark-muted mb-1">Khách hàng: <span className="text-dark-text font-bold">{selectedCustomer.name}</span></p>
                <p className="text-sm text-dark-muted">Nợ cần thu: <span className="text-red-400 font-bold">
                  {selectedStore === 'ALL' || selectedStore === 'KHO_TONG' ? (selectedCustomer.totalDebt || 0).toLocaleString() : (selectedCustomer.storeDebts?.[selectedStore] || 0).toLocaleString()}đ
                  {selectedStore !== 'ALL' && selectedStore !== 'KHO_TONG' && ` (tại ${SHOP_LABELS[selectedStore] || selectedStore})`}
                </span></p>
              </div>

              <div>
                <label className="block text-sm font-medium text-dark-muted mb-1">Số tiền thu</label>
                <div className="relative">
                  <input
                    type="number"
                    required
                    max={selectedStore === 'ALL' || selectedStore === 'KHO_TONG' ? selectedCustomer.totalDebt || 0 : (selectedCustomer.storeDebts?.[selectedStore] || 0)}
              ...
              <div>
                <label className="block text-sm font-medium text-dark-muted mb-1">Hình thức thanh toán</label>
                <select
        ...
        <div className="fixed inset-0 bg-black/80 backdrop-blur-sm flex items-center justify-center z-50 p-4">
          <div className="bg-dark-card border border-dark-border rounded-xl w-full max-w-md shadow-2xl overflow-hidden">
            ...
            <form onSubmit={handleCreateCustomer} className="p-4 space-y-4">
              ...
              <div>
                <label className="block text-sm font-medium text-dark-muted mb-1">Hạng / Nhóm khách</label>
      ...
      {isCustomerGroupModalOpen && (
        <div className="fixed inset-0 bg-black/80 backdrop-blur-sm flex items-center justify-center z-[60] p-4">
          <div className="bg-dark-card border border-dark-border rounded-xl w-full max-w-sm shadow-2xl overflow-hidden">
            ...
            <form onSubmit={handleCreateCustomerGroup} className="p-4 space-y-4">
              <div>
                <label className="block text-sm font-medium text-dark-muted mb-1">Tên nhóm</label>
                <input
                  type="text"
                  required
