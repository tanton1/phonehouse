...
const RECEPTION_TYPE_MAP: Record<
  string,
...
export default function NguonHang() {
  ...
  const filteredDevices = state.devices.filter((d) => {
    ...
    if (dateRange !== "ALL") {
      const deviceDate = d.receptionDate
        ? new Date(d.receptionDate)
        : d.importDate
          ? new Date(d.importDate)
          : null;
      ...
      if (dateRange === "TODAY") {
        ...
        matchesDate =
          deviceDate.getMonth() === lastMonth.getMonth() &&
          deviceDate.getFullYear() === lastMonth.getFullYear();
      } else if (dateRange === "CUSTOM") {
        if (customDateFrom)
          matchesDate = matchesDate && deviceDate >= new Date(customDateFrom);
        if (customDateTo)
          matchesDate = matchesDate && deviceDate <= new Date(customDateTo);
      }
    }
    ...
    <div className="space-y-6">
      ...
      <div className="bg-dark-card p-1 rounded-lg border border-dark-border overflow-hidden">
        <div className="flex overflow-x-auto p-1 gap-2">
          {Object.entries(RECEPTION_TYPE_MAP).map(
            ([key, { label, icon: Icon }]) => (
              <button
                key={key}
                onClick={() => {
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
        <div className="bg-dark-card p-4 rounded-xl border border-dark-border">
          <p className="text-[10px] font-bold text-dark-muted uppercase mb-1">
            Số lượng tồn
          </p>
        ...
        <div className="bg-dark-card p-4 rounded-xl border border-dark-border">
          ...
          <p className="text-lg font-bold text-neon-cyan">
            {filteredDevices
              .reduce((sum, d) => sum + d.importPrice, 0)
        ...
        <div className="bg-dark-card p-4 rounded-xl border border-dark-border md:col-span-2">
          ...
          <p className="text-sm text-dark-text">
            {activeTab === "IMPORT" &&
              "Máy nhập số lượng lớn từ các Nhà cung cấp (NCC). Đây là nguồn hàng chính để kinh doanh."}
            {activeTab === "TRADE_IN" &&
              "Máy thu lại từ khách hàng lẻ khi họ có nhu cầu lên đời máy."}
            {activeTab === "WARRANTY" &&
              "Máy do các Shop gửi về hoặc khách mang đến để xử lý các vấn đề phát sinh sau bán hàng."}
            {activeTab === "SERVICE" &&
              "Khách hàng mang máy đến sửa chữa (không phải máy của hệ thống bán ra)."}
            {activeTab === "SHOP_TRANSFER" &&
              "Điều chuyển hàng hóa giữa các chi nhánh (ví dụ từ XStore về Kho Tổng để bảo trì)."}
          </p>
        </div>
      </div>
            ...
            {showColumnDropdown && (
              <div className="absolute right-0 mt-2 w-48 bg-dark-card border border-dark-border rounded-md shadow-lg z-10 p-2">
                {Object.entries({
                  ...
                  <label
                    key={key}
                    className="flex items-center p-2 hover:bg-dark-bg rounded cursor-pointer"
                  >
                    <input
                      type="checkbox"
      ...
      <div className="bg-dark-card rounded-xl shadow-sm border border-dark-border overflow-hidden">
        <div className="overflow-x-auto">
          <table className="dark-table hidden md:table">
            <thead>
              <tr>
                {visibleColumns.imei && (
              ...
              {filteredDevices.map((device) => {
                ...
                if (activeTask) {
                  const assignee = state.users.find(
                    (u) => u.id === activeTask.assigneeId,
                  );
                  currentLocation = assignee
                    ? `KT: ${assignee.name}`
                    : "Kho Tổng";
                ...
                const deviceTasks = state.tasks.filter(
                ...
                return (
                  <tr key={device.id}>
                    {visibleColumns.imei && (
                      <td className="px-6 py-4 whitespace-nowrap text-sm font-medium text-dark-text">
                        ...
                        >
                          ...
                          <span className="border-b border-transparent group-hover:border-neon-cyan">
                            {device.imei}
                          </span>
                        </button>
                      </td>
                    ...
                    {visibleColumns.notes && (
                      <td
                        className="px-6 py-4 text-sm text-dark-muted max-w-xs truncate"
                        title={device.notes}
                      >
                        {device.notes}
                      </td>
            ...
            {filteredDevices.length === 0 ? (
              ...
              filteredDevices.map((device) => {
                ...
                return (
                  <div
                    key={device.id}
                    className="bg-dark-bg border border-dark-border rounded-lg p-4 flex flex-col gap-2"
                  >
                    <div className="flex justify-between items-start">
                      <div>
                        ...
                        <div className="text-xs font-mono text-neon-cyan mt-1 flex items-center">
                          <Smartphone className="w-3 h-3 mr-1" />
                          {device.imei}
                        </div>
                      </div>
                      <span className="px-2 py-1 text-[10px] rounded-full bg-dark-card border border-dark-border text-dark-muted font-bold whitespace-nowrap">
                        {device.status.replace(/_/g, " ")}
                      </span>
                    </div>
                    ...
                    <div className="flex flex-wrap gap-2 text-xs mt-2">
                      {device.color && (
                        <span className="px-2 py-1 rounded bg-dark-card border border-dark-border text-dark-muted">
                          {device.color}
                        </span>
                      )}
                      {device.source && (
                      ...
                      <span className="px-2 py-1 rounded bg-dark-card border border-dark-border text-dark-text font-medium">
